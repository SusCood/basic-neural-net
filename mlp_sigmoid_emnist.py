...
class Network:
	...
	def create_empty_weights_biases(self):
		...
		# initialised w/ 1 zero to hold nonexistent weights for the first layer; makes indexing easier
		del_weights, del_biases = [0], [0]
	...
	def train(self, num_of_batches):
		...
		for batch_i in range(num_of_batches):
			...
			print(f"Batch {batch_i+1}: {time_taken} s, average cost {round(total_cost/self.BATCH_SIZE, 4)}, {correct}/{self.BATCH_SIZE} correct ({self.percent}%); {round((batch_i + 1)/num_of_batches * 100, 1)}% complete (~{round(time_taken/60 * (num_of_batches - batch_i - 1))} min left)")
...
if __name__ == "__main__":
	...
	with (
		...
		img_data, label_data, t_img_data, t_label_data = img_f.read(), label_f.read(), t_img_f.read(), t_label_f.read()

	IMG_NUM, T_IMG_NUM = getbytes(4, 4, True), getbytes(4, 4, True, True)
